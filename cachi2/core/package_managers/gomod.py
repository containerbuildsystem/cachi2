    ...
    if strict and vendor.is_dir():
        raise PackageRejected(
            ...
            solution=(
                ...
                "instead.\nIf you do want to keep using vendoring, please pass one of the required "
                "flags."
            ),
            docs=VENDORING_DOC,
        )
...
def _vendor_deps(run_params: dict, can_make_changes: bool, git_dir: str):
    ...
    if not can_make_changes and _vendor_changed(git_dir, app_dir):
        raise PackageRejected(
            ...
            solution=(
                f"Please try running `go mod vendor` and committing the changes.\nNote that you "
                "may need to `git add --force` ignored files in the vendor/ dir.\nAlso consider "
                "whether you really want the -check variant of the flag."
            ),
            docs=VENDORING_DOC,
        )
