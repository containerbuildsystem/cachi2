...
class _FastCopyFailedFallback(Exception):
    """Fast in-kernel copying failed fallback to regular copy."""
...
def _fast_copy(src: Path, dest: Path, *, follow_symlinks: bool = True) -> int:
    ...
    Copy data from source path to destination path using a a high-performance copy_file_range(2)
