...
def test_get_pip_metadata_from_pyproject_toml(
    ...
    pyproject_toml.check_dynamic_version.return_value = dynamic_version
    ...
    assert "Resolved package name: foo" in caplog.messages

    if dynamic_version:
        assert "Dynamic version parsing from pyproject.toml is not supported" in caplog.messages
        assert version is None
    else:
        assert "Resolved package version: 0.1.0" in caplog.messages
...
def test_get_pip_metadata_from_setup_py(
    ...
    assert "Resolved package name: bar" in caplog.messages
    assert "Resolved package version: 0.2.0" in caplog.messages
...
def test_get_pip_metadata_from_setup_cfg(
    ...
    assert "Resolved package name: baz" in caplog.messages
    assert "Resolved package version: 0.3.0" in caplog.messages
...
def test_get_pip_metadata_from_remote_origin(
    ...
    if origin_exists:
        ...
        assert "Resolved package name: repo" in caplog.messages
        assert "Failed to resolve package version" in caplog.messages
    else:
