...
nox.options.sessions = ["bandit", "black", "flake8", "isort", "mypy", "unit-tests-python"]
...
@nox.session(name="unit-tests-python", python=parse_supported_python_versions())
...
def pip_compile(session: Session) -> None:
    ...
    pip-compile \
        ...
        pyproject.toml
    """
    cmd = f"podman run --rm --volume {PWD}:/cachi2:rw,Z --workdir /cachi2 docker.io/library/python:3.9-alpine sh -c {pip_compile_cmd}"
    session.run(*cmd.split(), pip_compile_cmd, silent=True, external=True)
