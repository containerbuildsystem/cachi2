...
This feature in cachi2 isn't concerned with pre-fetching those, but rather to provide a way to fetch individual artifacts
from maven repository. This is useful for cases where the build only needs specific artifacts from big projects, saving
time and bandwidth.
...
the download url for the artifact. [Maven documentation][maven-repo-layout] describes how to turn these coordinates into
an actual download url. Here's an example:
